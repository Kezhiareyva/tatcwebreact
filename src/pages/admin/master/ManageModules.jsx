...
const ManageModules = () => {
  ...
  const [formData, setFormData] = useState({ id: '', code: '', name: '', description: '', status: 'ACTIVE' });
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Yakin ingin menghapus modul ini? (Soft delete)')) return;
    try {
      const res = await apiFetch(`/api/modules?id=${id}`, {
        method: 'DELETE'
      });
      const json = await res.json();
      if (json.success) fetchModules();
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const openModal = (item = null) => {
    if (item) {
      ...
      setFormData({ id: '', code: '', name: '', description: '', status: 'ACTIVE' });
    }
    setMessage('');
    setIsModalOpen(true);
  };
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Manage Modules</h1>
        <button onClick={() => openModal()} className="btn btn-primary" style={{ padding: '8px 16px', borderRadius: '8px' }}>+ Add Module</button>
      </div>
      ...
      {loading ? <p>Loading data...</p> : (
        <div style={{ background: 'var(--surface-color)', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {modules.length === 0 ? (
                <tr><td colSpan="5" style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>Belum ada data modul.</td></tr>
              ) : modules.map(m => (
                <tr key={m.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  <td style={{ padding: '1rem', fontWeight: 'bold' }}>{m.code}</td>
                  <td style={{ padding: '1rem' }}>{m.name}</td>
                  <td style={{ padding: '1rem' }}>{m.description?.substring(0, 50)}{m.description?.length > 50 ? '...' : ''}</td>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '2rem' }}>
          <div style={{ background: 'var(--surface-color)', padding: '2rem', borderRadius: '12px', width: '100%', maxWidth: '600px' }}>
            <h2 style={{ marginBottom: '1.5rem', fontSize: '1.25rem' }}>{formData.id ? 'Edit Module' : 'Add Module'}</h2>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input type="text" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} required style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--border-color)', background: 'transparent', color: 'var(--text-main)' }} placeholder="e.g. MOD-01" />
                </div>
                <div style={{ flex: 2 }}>
